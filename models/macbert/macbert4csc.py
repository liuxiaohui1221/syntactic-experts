...
from base_model import CscTrainingModel, FocalLoss
...
class MacBert4Csc(CscTrainingModel, ABC):
    ...
    def forward(self, texts, cor_labels=None, det_labels=None):
        ...
        if text_labels!=None and encoded_text['input_ids'].shape[1]!=text_labels.shape[1]:
            encoded_text = self.tokenizer(cor_labels, padding=True, return_tensors='pt')
            text_labels = None
