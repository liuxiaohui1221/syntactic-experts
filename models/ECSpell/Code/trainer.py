...
from models.ECSpell import csc_evaluation
from models.ECSpell.csc_evaluation.common_utils import set_logger
from models.ECSpell.csc_evaluation.evaluate_utils import EvalHelper

from models.ECSpell.Code.pipeline import tagger
from models.ECSpell.Code.processor import Processor
from models.ECSpell.Code.common_utils import setSeed
from models.ECSpell.Code.DataAugment.text_utils import is_chinese_char
from models.ECSpell.Code.data_processor import attack_generation
from models.ECSpell.Code.evaluate import preprocess, postprocess
...
class TestArgs:
    def __init__(self, test_filename, labels, processor, result_dir):
        all_pairs, src_sents, clean_src, vocab = preprocess(test_filename)
        self.labels = labels
        ...
        self.vocab = vocab
        self.result_dir = os.path.join(result_dir, "predicts")
...
class Trainer:
        ...
        if resume_from_checkpoint is not None and os.path.isfile(
            ...
            if not args.ignore_data_skip:
                ...
                if self.is_local_process_zero() and not args.disable_tqdm:
                    steps_trained_progress_bar = tqdm(total=steps_trained_in_current_epoch, position=0, leave=True)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        #     logits, logits_2 = outputs["logits"], outputs_2["logits"]
        #     ce_loss = 0.5 * (model.loss_fct(logits.view(-1, model.num_labels), labels.view(-1)) +
        #                      model.loss_fct(logits_2.view(-1, model.num_labels), labels.view(-1)))
        #     pad_mask = (inputs["attention_mask"] == 0).unsqueeze(-1)

        #     kl_loss = compute_kl_loss(logits, logits_2, pad_mask)
    ...
    def evaluate(
        ...
        # do test while eval
        if self.test_args:
            official_metrics = self.do_test()

            # To be JSON-serializable, we need to remove numpy types or zero-d tensors
            official_metrics = denumpify_detensorize(official_metrics)

            # Prefix all keys with metric_key_prefix + '_'
            for key in list(official_metrics.keys()):
                if not key.startswith("test_"):
                    official_metrics[f"test_{key}"] = official_metrics.pop(key)
            official_metrics.update(speed_metrics("test", start_time, output.num_samples))
            self.log(official_metrics)

        return output.metrics

    def do_test(self):
        """
        Running prediction and return metrics
        """
        result_dir = os.path.join(self.test_args.result_dir, f"{PREFIX_CHECKPOINT_DIR}-{self.state.global_step}")
        if not os.path.exists(result_dir):
            os.makedirs(result_dir)
        result_filename = os.path.join(result_dir, "predict.result")
        # tag_sentences = tagger(
        #     self.model, self.tokenizer, self.test_args.clean_src, self.model.device,
        #     self.test_args.processor.pinyin_processor, self.test_args.labels, self.prefix_config)
        tag_sentences = tagger(self.model, self.tokenizer, self.test_args.clean_src, self.test_args.processor.vocab_processor, use_word=True,
                               use_pinyin=True, pinyin_processor=self.test_args.processor.pinyin_processor,
                               device=self.model.device, labels=self.test_args.labels)
        outputs = []
        for sentence, tag_sentence in zip(self.test_args.clean_src, tag_sentences):
            sentence = list(sentence)
            for tag_dict in tag_sentence:
                if tag_dict == []:
                    break
                # tag_id = int(tag_dict['entity'].split('_')[-1])
                # tag_token = self.test_args.labels[tag_id]
                tag_token = self.test_args.labels[tag_dict['entity']]
                # if the length of token is over 1, skip(not chinese)
        ...
        # postprocess
        outputs = postprocess(outputs, self.test_args.src_sents, self.test_args.vocab)

        with open(result_filename, 'w', encoding='utf-8') as f:
            for index, line in enumerate(outputs):
                f.write('\t'.join(self.test_args.all_pairs[index] + [line]))
                f.write('\n')

        results = []
        for (line_src, line_tgt), line_pred in zip(self.test_args.all_pairs, outputs):
    ...
    def evaluation_loop(
        ...
        if self.compute_metrics is not None:
            src_sents, gold_sents, pre_sents = [], [], []
            evaluation_results = []
            # Construct src_sents
            src_sents = self.eval_helper.texts
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if logits is not None and labels is not None and self.compute_metrics is not None:
                # Inference
                batch_gold_sents, batch_pre_sents = inference(src_sents, logits, self.eval_helper, self.model.config,
                                                              step, self.args.eval_batch_size)
                gold_sents.extend(batch_gold_sents)
                pre_sents.extend(batch_pre_sents)
                logits = None
                labels = None
                torch.cuda.empty_cache()
        ...
        if self.compute_metrics is not None and evaluation_results is not None:
            inference_start_time = time.time()
            if len(src_sents) != 0 and len(gold_sents) != 0 and len(pre_sents) != 0:
                vocab = []
                for i in range(len(src_sents)):
                    src_sents[i] = src_sents[i]
                    sent = src_sents[i]
                    ...
                    vocab.append(line)
                res = []
                for pre, src, v in zip(pre_sents, src_sents, vocab):
                    assert len(pre) == len(src) == len(v)
                    pre = list(pre)
                    src = list(src)
                    # restore not chinese characters
                    for i in range(len(pre)):
                        if v[i] == 1 or pre[i] == "[UNK]":
                            pre[i] = src[i]
                    res.append(pre)
                evaluation_results = [["".join(x), "".join(y), "".join(z)] for x, y, z in
                                      zip(src_sents, gold_sents, res)]
                metrics = _compute_metrics(evaluation_results, self.eval_helper)
...
def _compute_metrics(results: Union[List, numpy.ndarray], helper: EvalHelper,print_result=False):
    dacc, dpre, drec, df1, cacc, cpre, crec, cf1 = helper.compute(results, print_result=print_result)
    return {
        "dacc": round(dacc, 4), "dpre": round(dpre, 4), "drec": round(drec, 4), "df1": round(df1, 4),
        "cacc": round(cacc, 4), "cpre": round(cpre, 4), "crec": round(crec, 4), "cf1": round(cf1, 4),
    }


def convert_ids_to_tokens(src, gold, preds, config, unique_tag, offset=1):
    """convert the prediction ids to labels using config"""
    gold_sents, pre_sents = [], []
    for src_sent, gold_ids, pre_ids in zip(src, gold, preds):
        src_sent = src_sent
        gold_ids = gold_ids[1:-1]

        if len(src_sent) != len(gold_ids):
            pre_sents.append(src_sent)
            gold_ids = gold_ids[:len(src_sent)]
        else:
            pre_ids = pre_ids[offset:len(src_sent) + offset]
            pre_sent = convert_label_ids_to_tokens(src_sent, pre_ids, config, unique_tag)
            pre_sents.append(pre_sent)
        gold_sent = convert_label_ids_to_tokens(src_sent, gold_ids, config, unique_tag)

        gold_sents.append(gold_sent)
    return gold_sents, pre_sents


def convert_ids_to_tokens_single_sent(src_sent, gold_ids, pre_ids, config, unique_tag, offset=1):
    src_sent = src_sent[1:-1]
    gold_ids = gold_ids[1:-1]

    assert len(src_sent) == len(gold_ids)

    gold_sent = convert_label_ids_to_tokens(src_sent, gold_ids, config, unique_tag)
    pre_ids = pre_ids[offset:len(src_sent) + offset]
    pre_sent = convert_label_ids_to_tokens(src_sent, pre_ids, config, unique_tag)

    return gold_sent, pre_sent
...
def inference(src: Union[List, numpy.ndarray],
              logits: torch.Tensor,
              helper: EvalHelper,
    ...
    preds = logits
    scores = (torch.exp(preds) / torch.exp(preds).sum(-1, keepdims=True))

    gold, unique_tag = helper.labels, helper.unique_tag
    src = src[step * batch_size:]
    gold = gold[step * batch_size:]
    preds = scores.argmax(axis=-1).tolist()

    gold_sents, pre_sents = convert_ids_to_tokens(src, gold, preds, config, unique_tag=unique_tag)
