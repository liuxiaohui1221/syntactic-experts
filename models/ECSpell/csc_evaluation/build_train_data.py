...
def build_corpus(filenames, val_size, save_dir, use_test_as_val=False, test_files=None):
    ...
    with open(os.path.join(save_dir, 'preliminary_extend_train_ecsspell.txt'), 'w', encoding='utf-8') as f:
        f.writelines(train)

    with open(os.path.join(save_dir, 'preliminary_val_ecsspell.txt'), 'w', encoding='utf-8') as f:
        f.writelines(val)
    return
