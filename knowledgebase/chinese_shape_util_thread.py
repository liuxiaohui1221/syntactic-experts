...
class ChineseShapeUtil:
    ...
    def getSimilarityShape(self,match_char,topN=10):
        # 获取最接近的汉字
        # 缓存中查找
    ...
    def save_sim_shape_dict(self):
        ...
        print('saved dict size:',len(self.sim_shape_dict))
    def _save_all_sim_shape_dict(self):
        n=0
        for chinese in self.chinese_dict:
            self.getSimilarityShape(chinese)
            n+=1
            print('dict,n:',len(self.chinese_dict),n)
        self.save_sim_shape_dict()

    def multi_thread_compute_sim_shape(self,n,i):
        for index,chinese in enumerate(self.chinese_dict):
            if index % n != i:
                continue
            self.getSimilarityShape(chinese)
            print('index,size:',index,len(self.sim_shape_dict))
    def save_all_sim_shape(self,n=16):
        tlist=[]
        for i in range(n):
            t1 = threading.Thread(target=self.multi_thread_compute_sim_shape, args=(8,i),name='thread'+str(i))
            t1.start()
            tlist.append(t1)
        for t in tlist:
            t.join()
