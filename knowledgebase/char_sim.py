...
def parse_args():
    ...
    parser.add_argument('multiargs', nargs='*', type=str, default="好",
        ...
        for i, c1 in enumerate(args.multiargs):
            for c2 in args.multiargs[i:]:
                if c1 != c2:
                    ...
                    print(c.pronunciation_similarity(c1, c2))
    else:
        for i, c1 in enumerate(args.multiargs):
            for c2 in args.multiargs[i:]:
                if c1 != c2:
                    # print(f'For character pair ({c1}, {c2}):')
                    print(c.shape_similarity(c1, c2, as_tree=True))
                    print(c.pronunciation_similarity(c1, c2))
